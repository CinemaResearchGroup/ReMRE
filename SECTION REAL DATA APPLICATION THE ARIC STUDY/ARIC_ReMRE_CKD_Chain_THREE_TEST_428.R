...
if(K == 1){
	...
	size_pstep_vec <- c(
		1e-1, 									### beta00
		rep(1e-1, K), 							### beta0k_vec
		rep(7e-2, p), 							### beta0_vec
		c(rep(2e-1, 2), rep(2e-1, (p - 2) * K)),### betak_mat
		rep(3e-2, K),							### phi_vec
		4e-3, 									### logtheta
		rep(9e-1, K)							### sigma2_vec
	)
	...
	size_pstep_vec <- c(
		...
		rep(8e-2, p),						### p for D
		rep(5e-1, p), 						### p for MI
		rep(5e-1, 3), 1e0, rep(5e-1, 4), 	### p for HF
		rep(5e-1, p), 						### p for SK
		8e-2, 8e-2, 8e-2,					### phi_vec
		8e-2,								### logtheta
		2e-1, 5e-1, 2e-1, 					### sigma2_vec
		8e-2)								### rho
...
size_istep_mat <- matrix(c(rep(1.2e0, n),
						   rep(1.2e0, n),
						   rep(1.2e0, n)), 
						n, K, byrow = FALSE)
